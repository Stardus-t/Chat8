...
bool TcpChatServer::send_private_msg(const std::string& target_name, const std::string& msg, std::shared_ptr<ClientSession> sender){
	//通过RPC获取目标用户的UID
	std::cout << "发送获得UID请求" << std::endl;
	auto reply = StatusGrpcClient::GetInstance()->GetUserUid(target_name);
	if (reply.error()) {
...
bool TcpChatServer::check_user(const std::string& target_name, std::shared_ptr<ClientSession> sender) {
	//通过RPC获取目标用户的UID
	std::cout << "发送获得UID请求" << std::endl;
	auto reply = StatusGrpcClient::GetInstance()->GetUserUid(target_name);
	if (reply.error()) {
