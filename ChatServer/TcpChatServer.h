...
public:
	...
	//发送私聊消息
	bool send_private_msg(const std::string &target_name,const std::string &msg, std::shared_ptr<ClientSession> sender);

	//检查用户是否存在
	bool check_user(const std::string& target_name, std::shared_ptr<ClientSession> sender);
